...
import com.shinyst.coalpoolmobile.data.models.PUBLIC_KEY_LEN
import com.shinyst.coalpoolmobile.data.models.SIGNATURE_LEN
...
class SolanaRepository : ISolanaRepository {
    ...
    override fun getSolTransferTransaction(latestBlockhash: String, from: String, to: String, amount: ULong): Transaction? {
        try {
            val encodedTxn = uniffi.coalpoolmobileffi.getTransferLamportsTransaction(latestBlockhash, from, to, amount)
            var decodedTxn = Base64.decode(encodedTxn)
            val txn = Transaction.from(decodedTxn)
            return txn
        } catch (e: uniffi.coalpoolmobileffi.CoalPoolMobileFfiException) {
    ...
    override fun getSolTransferTransactionWithFeePayer(latestBlockhash: String, from: String, to: String, feePayer: String, amount: ULong): Transaction? {
        try {
            val encodedTxn = uniffi.coalpoolmobileffi.getTransferLamportsTransactionWithFeePayer(latestBlockhash, from, to, feePayer, amount)
            var decodedTxn = Base64.decode(encodedTxn)
            val txn = Transaction.from(decodedTxn)
            return txn
        } catch (e: uniffi.coalpoolmobileffi.CoalPoolMobileFfiException) {
